    ...
    if (filtered.length === 0) {
      galleryGrid.innerHTML = `
        <div style="grid-column: 1 / -1; text-align: center; padding: 4rem 1rem; color: var(--text-muted);">
          ...
          <h3>No matching artworks found</h3>
          <p style="font-size: 0.9rem; margin-top: 0.5rem;">Try searching with different keywords or upload a new artwork!</p>
        </div>
      `;
      return;
    }
  ...
  function openDetailModal(item, updateHash = true) {
    ...
    if (item.isCustom) {
      ...
      deleteBtn.onclick = async () => {
        if (confirm("Are you sure you want to delete this artwork from your portfolio collection?")) {
    ...
    btnShare.addEventListener("click", () => {
      if (activeArtwork) {
        ...
        if (navigator.clipboard) {
          ...
          showToast("Direct artwork link copied to clipboard!");
        } else {
          prompt("Direct artwork link:", directUrl);
        }
      }
    });
  }
